...
class Model(tf.Module):
    ...
    def train(self, lr, cliprange, obs, returns, masks, actions, values, neglogpac_old, states=None, apply_grad=True):
        grads, pg_loss, vf_loss, entropy, approxkl, clipfrac, loss = self.get_grad(
        ...
        return pg_loss, vf_loss, entropy, approxkl, clipfrac, loss
    ...
    def get_grad(self, cliprange, obs, returns, masks, actions, values, neglogpac_old):
        ...
        return grads, pg_loss, vf_loss, entropy, approxkl, clipfrac, loss

    @tf.function
    def get_loss(self, cliprange, obs, returns, masks, actions, values, neglogpac_old):
        # Here we calculate advantage A(s,a) = R + yV(s') - V(s)
        # Returns = R + yV(s')
        advs = returns - values
