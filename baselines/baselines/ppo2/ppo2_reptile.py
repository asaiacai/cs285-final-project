...
def learn(*, network, env, total_timesteps, eval_env = None, seed=None, nsteps=2048, ent_coef=0.0, lr=3e-4,
            vf_coef=0.5,  max_grad_norm=0.5, gamma=0.99, lam=0.95, eps_reptile=0.1,
    ...
    **network_kwargs:                 keyword arguments to the policy / network builder. See baselines.common/policies.py/build_policy and arguments to a particular type of network
    ...
    for update in range(1, nupdates+1):
        ...
        if states is None: # nonrecurrent version
            ...
            for _ in range(noptepochs): # number of meta iterations                
                new_vars = []
                old_vars = [tf.identity(w) for w in weights]
                # iterate through tasks
                for tstart in np.random.permutation(range(0, nbatch, nsteps)):
                    tend = tstart + nsteps
                    task_obs, task_returns, task_masks, task_actions, task_values, task_neglogpacs = (arr[tstart:tend] for arr in (obs, returns, masks, actions, values, neglogpacs)) 
                    # Randomize the indexes
                    inds = np.random.permutation(np.arange(nsteps))
                    # do k iterations of SGD/Adam
...
def average_vars(variables):
    return [sum(v) / len(v) for v in zip(*variables)]